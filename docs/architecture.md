...
scraping-gateway/
  ...
  references/
    ...
    02-l3-service-architecture.md  ← L3 Edge CDP
...
L3  真实浏览器          → Edge CDP (Windows 侧, 真实指纹)
L4  人工介入            → 需要登录/验证码的站点
...
L3 层的独特价值：通过 Windows 侧的真实 Edge 浏览器（非自动化标志启动），获得完全原生的浏览器指纹，绕过大部分反爬检测。
